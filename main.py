...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Animate your images and narration as a whiteboard or a synchronized split-page storybook.")
    parser.add_argument("--mode", choices=["whiteboard", "storybook"], default="whiteboard")
    parser.add_argument("--text", type=Path, help="Story script, with blank lines between paragraphs")
    parser.add_argument("--timestamps", type=Path, help="Whisper word-level JSON or SRT timing plan")
    parser.add_argument("--storybook-font", type=Path, default=Path(__file__).resolve().parent / "assets/fonts/storybook.ttf")
    parser.add_argument("--page-turn", choices=["curl", "fade", "none"], default="curl")
    parser.add_argument("--page-turn-duration", type=float, default=1.2)
    parser.add_argument("--paper-style", choices=["ivory", "white", "warm"], default="ivory")
    parser.add_argument("--text-pen", action="store_true")
    parser.add_argument("--no-highlight", action="store_true")
    parser.add_argument("--sketch-speed", type=float, default=1.5)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.mode == "storybook":
            if not args.text or not args.text.is_file():
                raise ValueError("Storybook mode requires --text <script.txt>.")
            from src.storybook_engine import export_storybook
            manifest = export_storybook(images, args.text, args.audio, args.output, timestamps=args.timestamps, fps=args.fps, overwrite=args.overwrite, progress=progress, hand=not args.no_hand, text_pen=args.text_pen, highlight=not args.no_highlight, font_path=args.storybook_font, paper=args.paper_style, transition=args.page_turn, transition_duration=args.page_turn_duration, sketch_speed=args.sketch_speed)
        elif len(images) == 1 and durations is None:
