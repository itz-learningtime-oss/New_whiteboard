...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (project.duration > 180 || paragraphs(project.script).length > 20) throw new Error("Online MP4 rendering supports up to 3 minutes and 20 pages. Use the offline kit for longer stories.");
    ...
    const executable = process.env.PYTHON_BINARY || "python3";
    ...
    child.on("close", (code, signal) => {
      ...
      void update(code === 0 ? { status: "complete", progress: 100 } : { status: cancelled ? "cancelled" : "failed", error: cancelled ? "Rendering was cancelled or exceeded 15 minutes." : (stderr.trim().slice(-1600) || "The renderer stopped unexpectedly.") });
    });
