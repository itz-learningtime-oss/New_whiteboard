"""Decode once, count PCM samples with wave, and build a drift-free timeline."""
...
AUDIO_EXTENSIONS = {".wav", ".mp3", ".m4a", ".aac", ".flac", ".ogg", ".opus", ".aif", ".aiff", ".webm"}
...
def analyze_audio(source: Path, work_dir: Path, ffmpeg: str | None = None) -> AudioInfo:
    ...
    try:
        ...
        # Read the decoded PCM sample count directly, without millisecond rounding
        # or the deprecated audioop dependency used by older audio libraries.
        with wave.open(str(target), "rb") as decoded:
            info = AudioInfo(source, target, decoded.getnframes(), decoded.getframerate(), decoded.getnchannels())
